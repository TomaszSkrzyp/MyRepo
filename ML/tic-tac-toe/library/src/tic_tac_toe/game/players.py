...
class Player(metaclass=abc.ABCMeta):
    ...
    def make_move(self, game_state: GameState) -> GameState:
        if self.mark is game_state.current_mark:
            ...
            raise InvalidMove("No other moves available")
        else:
            raise InvalidMove("its{game_state.current_mark}'s turn to move, not {self.mark}'s turn")
    @abc.abstractmethod
    def get_move(self, game_state: GameState) -> Move:
        pass
class ComputerPlayer(Player, metaclass=abc.ABCMeta):
    def __init__(self, mark: Mark, delay_seconds: float = 0.5) -> None:
        super().__init__(mark)
